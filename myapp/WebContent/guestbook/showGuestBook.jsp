...
<body bgcolor="#996600">
	<div align="center">
		...
		<table width="520" cellspacing="0" cellpadding="3">
			<tr bgcolor="#F5F5F5">
				<td><font size="2"><b><%=login.getName()%></b>님 환영합니다. </font>
				</td>
					...
					<table bgcolor="#F5F5F5">
						<tr>
							...
							<td width="225"><img src="img/face.bmp" border="0" alt="이름">
								<a href="mailto:<%=writer.getEmail() %>"> <%=writer.getName() %></a></td>
						...
						<tr>
							<td>IP : <%=bean.getIp() %></td>
							<td><%=bean.getRedate()+" "+bean.getRegtime() %></td>
							<td>
								<%
									//로그인 id와 방명록 쓴 사람의 id가 동일하다면 수정, 삭제 모드 활성화
									boolean chk = login.getId().equals(writer.getId());
									if(chk || login.getGrade().trim().equals("1")){ //동일인이거나 관리자일 경우
										if(chk){
				...
				<td>
					<table width="500">
						<tr>
							...
							%>
								<a href="commentProc.jsp?flag=delete&cnum=<%=cbean.getCnum()%>">[삭제]</a></td>
							<%	
								}
							%>
						</tr>
