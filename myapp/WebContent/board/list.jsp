...
<%
	...
	int numPerPage = 10; //page당 record 수
	int pagePerBlock = 15; //block당 page 수(목록 수)
	int totalPage = 0; //총 page 수=(올림)총 게시물 수 / numPerPage
	int totalBlock = 0; // 총 block 수=(올림)totalPage / pagePerBlock
	int nowPage = 1; // 현재 page
	int nowBlock = 1; // 현재 block
	
	//page에 보여질 게시물 갯수 값 5, 10, 15, 30
	if (request.getParameter("numPerPage") != null) {
		numPerPage = Integer.parseInt(request.getParameter("numPerPage"));
	}

	int start = 0; //tblboard에 select 시작번호
	int end = numPerPage; //tblboard에 select 끝번호

	String keyField = "", keyWord = ""; //검색에 필요한 값
	//검색 할때
...
<script type="text/javascript">
	...
	function check() {
		if (document.searchFrm.keyWord.value == "") { //검색을 안하면
			alert("검색어를 입력하세요.");
					...
					<table cellspacing="0">
						...
						<%
							for(int i = 0; i < numPerPage; i++){
								...
								int bcount = cmgr.getBCommentCount(num); //댓글의 갯수
						%>
						<tr	align="center">
