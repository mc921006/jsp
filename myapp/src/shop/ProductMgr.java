...
public class ProductMgr {
	...
	public Vector<ProductBean> getProductList() {
		...
		try {
			...
			sql = "SELECT no, name, price, date, stock FROM tblproduct "
					+ "ORDER BY no DESC"; //order by no desc를 order by 1 desc를 해도 가능. select from 사이 순서(1~5) 번호
	...
	public boolean updateProduct(HttpServletRequest req) {
		...
		try {
			...
			if (multi.getFilesystemName("image") == null) { //파일은 수정 안함.
				...
				sql = "UPDATE tblproduct SET name = ?, price = ?, detail = ?, stock = ?, image = ?"
						+ " WHERE no = ?";
				pstmt = con.prepareStatement(sql);
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, multi.getParameter("name"));
	...
	//Product Delete
	public boolean deleteProduct(int no) { //no : 상품 번호
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		boolean flag = false;
